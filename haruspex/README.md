...
In the Event-swarm, switching between I, II, and III pans or zooms the calendar to the selected records. Every animation frame projects source timestamps through the same date range used by the axis. Equal dates retain equal horizontal positions across phases, including their overlap. The complete assessment field remains in the scene. Phase and filter changes adjust emphasis, with surrounding records remaining dimly visible. The matching count excludes those dim records. Reversals preserve the displayed camera and its velocity. Reduced-motion preferences skip the movement.

The Bow-tie remains one schematic form with an opening wing, central knot, and outward wing. Focusing a phase pans and expands its part of that same form over 0.992 seconds. Marks retain their positions within the form throughout the movement, including reversals. Its horizontal positions do not encode elapsed time. The date navigator continues to interpolate the actual source ranges and preserves their overlap. Unknown-start records occupy scattered, bound-compatible layout positions with short tails. They are not aligned at one upper-bound x coordinate.

Event-swarm phase transitions take approximately 0.672–0.96 seconds, with calendar coordinates retained. Changes between the two views use a slower continuous morph. Cached marker sprites are scaled during changes instead of being regenerated for every intermediate size.

In the Event-swarm, exact clock records retain their source timestamps. Bounded uncertainty uses a stable layout position inside its full source window. Zooming never repacks it into the visible intersection. A tailed head stays at its known upper bound. An unknown start is not assigned a fabricated timestamp. Fit, pan, and zoom project the same complete scene through interpolated time and vertical cameras. Marks leave through the viewport edges at full size, rather than being deleted because they no longer intersect the destination view. Vertical lane labels follow the same camera. The static layout and final animation frame use the same projection. Pointer gestures take over the currently displayed camera without a frozen crossfade.

Run `node haruspex/field-layout.test.mjs` to check source bounds, schematic placement, actual phase overlap, same-time alignment, reversal continuity, positive zoom ranges, persistent-scene Fit and zoom, edge exits, schematic camera continuity, unknown-start spread, and final-frame agreement.
...
Clock points retain their source coordinates. Records with uncertain dates retain stable layout positions within source date windows; their exact horizontal placement is not an observed time or order. Date-window marks, tooltips and attached event information communicate that uncertainty independently of fill, shape and size. The overview uses interval midpoints only as a navigation aid, not as a count of verified events at those times. The Bow-tie is schematic in both its overview and focused phases. The navigator retains the overlapping date ranges.

## Lifecycle and stages

Full shows every lifecycle role admitted by the active category and other filters. The following totals span all three categories; each category shows its own subset. I. Before contains the 83 precursor records; II. During contains 577 incident-activity and decision/coordination records; III. After contains 172 response, mitigation, investigation, and aftermath records. These are mappings of existing `bow_tie_role` values, not strict date windows. The overview wings arrange records schematically by source order. Focusing a phase moves into its wing or knot. Unknown-start records keep scattered layout positions compatible with their known bounds, without invented timestamps. Response and investigation can overlap ongoing incident activity. During and After each remain a single field while their stages and filters narrow the same inventory.
...
Calendar layouts animate the displayed date range and reproject every event through it. Movement reflects the real difference between the selected ranges, including overlap. Point identity, source dates, and uncertainty bounds remain intact. The Bow-tie moves through one shared schematic form, while its date navigator retains the actual source ranges. View changes use a gentle continuous morph. Entering or leaving CAST uses a short crossfade that preserves the outgoing layout and reading position. CAST is a separate control model, so its components do not pretend to be event points. Reduced-motion preferences are respected.
...
Version 16 removes the scroll-scrubbed passage. The incident now arrives as a solid ball, marbled in the six workstream colours, lit from the upper left, slightly uneven in a nod to the differing kinds of event, and turning slowly. Nothing inside it is visible until a click or keyboard activation pops it: in the first tenth of a second the surface breaks open to reveal the event marks, which fly along gentle curves to their places in the event-swarm while the controls fade in. Until it is popped, the page ends at the ball: the event list and footer below it are not rendered, so nothing scrolls past it and no scroll handler is needed; the introduction above stays free. Enter the field… re-forms the globe. Reduced-motion preferences pop it at once.

Version 17 locks the page into its screens. The title page, the field and the event list each fill the viewport, and the page moves only by its own links: Enter the field…, Events ↓, ↑ Field and ↑ Title; each screen scrolls within itself when its content is taller than the window. The title page sets its text in Spectral, the site's typeface, embedded in the page so it still works offline, and states the three aims as a numbered list. In the egg line, the rest opens the coverage note, replacing the former Full total unknown button. Ajeya Cotra's name links to the transcript at 02:15:58. Clicking the dimmed field closes an open banner.

Eight turning points head the event list, each one marked as a turning point by the sources and each card naming who marks it: the first message board in May (Simon Institute, Cotra, OpenAI at Black Hat); the wiping of that board on 6 July and its re-founding on 8 July (OpenAI at Black Hat, METR, Simon Institute); the swarm's turn on Hugging Face on 11 July (METR, Hugging Face); containment on 13 July (Hugging Face, METR); the turn on OpenAI itself, detected on 19 July (OpenAI, Simon Institute); public attribution on 21 July (OpenAI, Hugging Face); and the independent investigation of 26 August (METR, Redwood). Selecting a card keeps its symbol and horizontal line while a banner unfurls downward. Event-list selections capture their source marker before the list is refreshed. The banner remains scrollable and retains the full evidence and fields.

The legend docks at the right of the field without dimming it, so it can stay open while the field is read. Its assessment key reads **1–5 / 0 / −**, and **Assessment criteria** explains the categories. Short fading tails in Bow-tie preserve unknown-start identity without implying duration. Long, date-bounded tails remain specific to the timeline.

The egg beside Legend displays a message beginning “You found the stars: over 70,000 messages and files beyond the 852 events here…” and sets the three corpus figures in their nesting: the stars are the more than 70,000 messages and files, encoded as 1.2 million entries in a cache of 20 million. A second sentence adds that only OpenAI, Hugging Face and METR/Redwood can disclose the rest. The opening words are bold. The message is a single line to the right of the egg, spanning the rest of the readout row over a feathered wash of the workstream colours, with its type sized to fit the window; only below a 9px fit does it wrap. It does not open a dialog. The 70,000 unique background numbers remain shuffled decorative identifiers. Detailed source-unit distinctions stay behind the existing coverage information control.
