...
The Bow-tie remains one schematic form with an opening wing, central knot, and outward wing. Focusing a phase pans and expands its part of that same form over 1.24 seconds. Marks retain their positions within the form throughout the movement, including reversals. Its horizontal positions do not encode elapsed time. The date navigator continues to interpolate the actual source ranges and preserves their overlap. Unknown-start records occupy scattered, bound-compatible layout positions with short tails. They are not aligned at one upper-bound x coordinate.

Event-swarm phase transitions take approximately 0.84–1.2 seconds, with calendar coordinates retained. Changes between the two views use a slower continuous morph. Cached marker sprites are scaled during changes instead of being regenerated for every intermediate size.
