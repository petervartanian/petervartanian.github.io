...
Version 16 removes the scroll-scrubbed passage. The incident now arrives as a slowly turning ball, lit from the upper left and slightly uneven in a nod to the differing kinds of event: the plotted event marks on its surface over a dense filling of circles, diamonds, triangles and six-point stars in the workstream colours. Only a click or keyboard activation pops it: the filling dissolves where it stands while the marks fly along gentle curves to their places in the event-swarm and the controls fade in. Scrolling leaves the ball in place. Enter the field… re-forms the globe. Reduced-motion preferences pop it at once.
