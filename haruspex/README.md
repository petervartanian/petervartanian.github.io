...
In the Event-swarm, switching between I, II, and III pans or zooms the calendar to the selected records. Every animation frame projects source timestamps through the same date range used by the axis. Equal dates retain equal horizontal positions across phases, including their overlap. Incoming and outgoing records grow or shrink at their calendar positions, with no artificial offscreen travel. Reversals preserve the displayed camera and its velocity. Reduced-motion preferences skip the movement.

The Bow-tie remains one schematic form with an opening wing, central knot, and outward wing. Focusing a phase pans and expands its part of that same form over 1.55 seconds. Marks retain their positions within the form throughout the movement, including reversals. Its horizontal positions do not encode elapsed time. The date navigator continues to interpolate the actual source ranges and preserves their overlap. Unknown-start records occupy scattered, bound-compatible layout positions with short tails. They are not aligned at one upper-bound x coordinate.

Event-swarm phase transitions take approximately 1.05–1.5 seconds, with calendar coordinates retained. Changes between the two views use a slower continuous morph. Cached marker sprites are scaled during changes instead of being regenerated for every intermediate size.

In the Event-swarm, exact clock records retain their source timestamps. Bounded uncertainty stays within the source window, and a tailed head stays at its known upper bound. An unknown start is not assigned a fabricated timestamp. The static layout and final animation frame use the same projection.

Run `node haruspex/field-layout.test.mjs` to check source bounds, schematic placement, actual phase overlap, same-time alignment, reversal continuity, positive zoom ranges, schematic camera continuity, unknown-start spread, and final-frame agreement.
