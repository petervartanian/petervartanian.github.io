...
Version 16 removes the scroll-scrubbed passage. The incident now arrives as a solid ball, marbled in the six workstream colours, lit from the upper left, slightly uneven in a nod to the differing kinds of event, and turning slowly. Its pieces are not visible until a click or keyboard activation pops it: in the first tenth of a second the surface breaks into circles, diamonds, triangles and six-point stars, which dissolve where they stand while the true event marks fly along gentle curves to their places in the event-swarm and the controls fade in. Scrolling leaves the ball in place. Enter the field… re-forms the globe. Reduced-motion preferences pop it at once.
...
The egg beside Legend displays a message beginning “You found it! Beyond these 852 events…” and sets the three corpus figures in their nesting: more than 70,000 messages and files, encoded as 1.2 million entries in a cache's 20 million. A second sentence adds that without fuller disclosure from OpenAI, Hugging Face and METR/Redwood, this investigation can go no further. The opening words are bold. The message is a single line to the right of the egg, spanning the rest of the readout row over a feathered wash of the workstream colours, with its type sized to fit the window; only below a 9px fit does it wrap. It does not open a dialog. The 70,000 unique background numbers remain shuffled decorative identifiers. Detailed source-unit distinctions stay behind the existing coverage information control.
