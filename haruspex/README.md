...
The renderer uses 5,000–22,000 tiny density marks according to the viewport. That number describes visual density, not events. A tiny egg tucked into the title-page footer unlocks the number-only Easter egg. Before it is found, hovering or tapping background points reveals nothing. Once unlocked, hover near a distant point or tap it to reveal its number; the egg can also hide the numbers again. The Legend has no star-number reveal control. These are stable decorative catalogue numbers, not historical event IDs, and have no incident details, dates, severity or evidence status. A spatial index handles hit testing without thousands of DOM elements. They do not contribute to the 832-record inventory or active filter counts. Their positions and opacity do not encode chronology, confidence, probability or causal relationships.
...
Investigate opens **An incident across boundaries**, a continuous white paper that introduces Causal Analysis based on Systems Theory (CAST), explains the control system, and develops interpretations of access, shared resources, incentives, escalation, containment and protections that held. It distinguishes public evidence already collected from missing operational records, interviews and experiments. Sparse numbered citations lead to source notes. A conceptual control/feedback figure explains the method without implying a proven causal sequence.
...
- `assets/mit-airi-official.svg`: official MIT AI Risk Initiative logo used for the risk-framework reference link and embedded in the self-contained application.
