...
Version 16 removes the scroll-scrubbed passage. The incident now arrives as a slowly turning globe made of the event marks themselves, circles, diamonds, triangles and six-point stars in their workstream colours with a fine dust between them, and a click or keyboard activation pops it: the marks fly along curved paths to their places in the event-swarm while the controls fade in. Scrolling on past the globe pops it too, so nothing depends on scroll position. Enter the field… re-forms the globe. Reduced-motion preferences pop it at once.
...
The egg beside Legend displays a message beginning “You found it! Beyond these 852 events…” and sets the three corpus figures in their nesting: more than 70,000 messages and files, encoded as 1.2 million entries — mostly fragments — in a cache of 20 million files and directories. It then turns to the disclosure gap: without fuller disclosure from OpenAI, Hugging Face and METR/Redwood, this investigation can go no further. The opening words are bold and underlined. The message is a single line of starlight across the readout; when it is wider than the strip it drifts slowly from side to side, pausing under the pointer, and it wraps only for readers who prefer reduced motion. It does not open a dialog. The 70,000 unique background numbers remain shuffled decorative identifiers. Detailed source-unit distinctions stay behind the existing coverage information control.
