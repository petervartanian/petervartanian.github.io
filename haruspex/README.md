...
This is application version 14.0.0. Severity scores and canonical records are unchanged; additional research is tracked separately; the 3.0.1 assessment revision only standardizes Near-miss spelling. The canonical dataset remains unchanged; reviewed assessments, corrected current evidence display, temporal bounds, ontology and visual conventions live in separate versioned layers. Full original fields remain available on each event.
...
The introduction uses superscript Roman endnotes for valuation, collaborating agents and later internal intrusions. Ajeya Cotra’s name retains its direct transcript link, as requested. The official AIRI mark has no caption. Context and Unresolved remain independent assessment fields.

Version 14 follows **introduction → sphere → event-swarm**. The large sphere has no visible text. It divides into rounded daughters along gently changing axes, then resolves into the actual filtered event objects. Reversing page scroll reverses the division. A cached surface texture avoids rebuilding gradients for every daughter on every frame. The keyboard-accessible sphere button advances to the field. Existing view navigation goes directly to its destination. Reduced-motion preferences use the static surface followed by the completed field.

Selected events unfurl into an anchored banner. Their workstream-colored marker and curved connector remain visible against a blurred background. The banner adapts above or below the marker on phones, supports keyboard navigation, and preserves access to dates, assessment rationale, all 45 fields, and source endnotes. A backdrop press or Escape closes it.

Introductory choices read **Enter the field… OR …Read the investigation**. Roman endnotes also replace source-link clusters in event details, coverage explanations, investigation appendices and reference dialogs. CAST is expanded in the investigation’s opening paragraph. Relevant specialist terms have a local “Terms used here” disclosure.

The hidden egg beside Legend opens with “You found it! Beyond these 852 explorable events…” and explains the distinct source units: over 70,000 messages and files, roughly 1.2 million raw message-board entries (mostly encoded file fragments), a wider collection of about 20 million files/directories, around 1,300 transcripts, and Hugging Face’s separate recovered-action count. These quantities overlap and cannot establish a full event count. The note identifies the disclosures needed from OpenAI, Hugging Face, METR/Redwood, Nightingale and the wiki operators. All 70,000 decorative star identifiers are unique and deterministically shuffled in space.
