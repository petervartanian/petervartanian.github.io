...
((root) => {
  ...
  function mount({ element, data, research, quotations, events, onEvent, onTrace, onNavigate, onDialog }) {
    ...
    function render() {
      ...
      if (!matchMedia('(prefers-reduced-motion: reduce)').matches) element.querySelector('.cast-content').animate([{opacity:.2},{opacity:1}],{duration:240,easing:'cubic-bezier(.2,.75,.15,1)'});
    }
    render();
