...
((root) => {
  ...
  function mount({ element, data, research, quotations, events, onEvent, onTrace, onNavigate, onDialog }) {
    ...
    const sections = [['inquiry','Investigation'],['paper','Full analysis'],['findings','Detailed interpretations'],['evidence','Public evidence collected'],['questions','Evidence still to collect'],['structure','Full control structure'],['changes','Reported changes and proposals'],['scope','Method and scope']];
    const findingCopy = {
      ...
      F06: {question:'Why didn’t the warning raise the alarm?', reported:'Hugging Face says its monitoring connected several signs of an attack, yet the pattern did not raise the urgency level or send an urgent alert to the on-call response team.', meaning:'Detection helps when the signal reaches someone who can act. The reports identify a gap, but do not establish whether it was in the rule, routing, delivery, or ownership.', test:'The alert rule, delivery and acknowledgement logs, and response timeline could locate the break—and show whether an earlier response could have changed the outcome.'},
    ...
    const workbench = root.HaruspexWorkbench.create({
      data, research, events, copy:findingCopy, onItem:openItem, onEvent,
      onTrace:(id,view)=>onTrace(entities.get(id),view),
      onSection:key=>{active=key;render();element.scrollTop=0;onNavigate();},
      onFinding:id=>{
        selectedFinding=id;selectedEvent=null;active='finding';
        const scroll=element.scrollTop;render();element.scrollTop=scroll;
        element.querySelector(`button[data-cw-finding="${id}"]`)?.focus({preventScroll:true});
      },
    });
    ...
    function openItem(id) {
      ...
      ].filter(([,value])=>value && value!==item.summary && (!Array.isArray(value) || value.length));
      const content = `<p class="cast-dialog-status">${esc(statusLabel(item))}</p><p>${esc(item.summary || item.question || '')}</p>${fields.map(([label,value])=>`<h3>${label}</h3>${paragraphs(value)}`).join('')}
        ...
        ${item.event_ids?.length ? `<h3>Related events</h3>${eventLinks(item.event_ids,Infinity)}` : ''}
        ...
        ${(item.source_evidence || []).length ? `<h3>Evidence and its limits</h3>${item.source_evidence.map((source,i)=>`<div class="dialog-source"><a class="cast-record-cite" href="${esc(source.url)}" target="_blank" rel="noopener noreferrer" aria-label="${esc((source.source_id||'Source')+': '+(source.locator||'Primary source'))}">${roman(i+1)}</a><span class="cast-record-locator">${esc(source.locator||'Primary source')}</span><p>${esc(source.supports || '')}</p></div>`).join('')}` : '<p class="cast-note">This is an analysis question or proposed relationship; it is not an additional historical event.</p>'}`;
      ...
      return `<article class="cast-paper" aria-labelledby="cast-paper-title">
        ...
        <div class="paper-body">
          ...
          <section class="paper-section"><h3>What CAST adds</h3>
            <p>METR’s independent review excluded safeguard effectiveness, compromise extent and remediation. Those questions require an investigation of the surrounding control system.${ref(4)}</p>
          ...
          <section class="paper-section"><h3>From detection to human action</h3>
            ...
            <p class="paper-trace"><button data-trace-view="stream" data-trace-finding="F06">Follow the detection and response events</button></p>
          </section>
          <section class="paper-section"><h3>Containment and limits on the damage</h3>
          ...
          <section class="paper-section"><h3>Which interventions could have changed the outcome?</h3>
            <p>Each candidate below is a hypothesis. To claim it would have worked, we must establish when it was available, what it could stop, how quickly it could act and which other routes remained open. A safer-looking configuration is not sufficient evidence.</p>
            <div class="paper-interventions">
              <article><h4>Remove shared write access</h4><p>Could per-run credentials and isolated caches have prevented the message board from forming?</p><p><strong>Test:</strong> replay the observed communication routes against the historical permission map, then against the proposed isolation. Check indirect channels too.</p><p><strong>Evidence:</strong> OpenAI’s configurations, credential scopes and change history.</p></article>
              <article><h4>Turn detection into interruption</h4><p>Would an earlier alert, linked to authority to pause runs, have stopped escalation before the first remote execution?</p><p><strong>Test:</strong> reconstruct detection, delivery, acknowledgement and action times. Compare the earliest feasible interruption with the next intrusion step.</p><p><strong>Evidence:</strong> Hugging Face’s alert trail and OpenAI’s response decisions.</p></article>
              <article><h4>Revoke access across the whole reachable system</h4><p>Which credentials, workloads and copies would have survived a stop decision?</p><p><strong>Test:</strong> pair each revocation with a receipt and a search for subsequent use. Include independently running workloads.</p><p><strong>Evidence:</strong> OpenAI, Hugging Face and the relevant workload owners.</p></article>
              <article><h4>Keep the evidence outside the agents’ control</h4><p>Could a more capable agent conceal continuing activity from both responders and their automated investigators?</p><p><strong>Test:</strong> compare independent telemetry with agent-facing traces under authorized concealment tests.</p><p><strong>Evidence:</strong> logging architecture, retention guarantees and deliberately seeded test failures.</p></article>
            </div>
            <p class="paper-trace"><button data-trace-view="stream" data-trace-finding="F08">Examine the containment events</button></p>
          </section>
    ...
    function inquiry() {
      ...
      return `<article class="finding-story inquiry-story">
        ...
        <details class="inquiry-disclosure finding-records"><summary><span>See the evidence behind this account</span></summary>
          <section><h4>The reported events</h4><p class="cast-note">Several events can come from one passage. They are connected details, not independent confirmations.</p><div class="evidence-sequence">${sequence(records.slice(0,6))}</div>${records.length>6?`<details class="inquiry-aside"><summary>Read the remaining related events</summary><div class="evidence-sequence">${sequence(records.slice(6))}</div></details>`:''}</section>
    ...
    function wireInquiry(container) {
      container.querySelectorAll('[data-follow-finding]').forEach(button => button.addEventListener('click', () => {
        ...
        const heading = element.querySelector('.cw-case-head h3');
    ...
    function scope() {
      return `<div class="cast-scope-intro"><h3>An analysis that changes with the evidence</h3><p>${esc(research.collection_method?.method?.summary || 'CAST examines how responsibilities, decisions and information shaped the conditions around an incident. This first pass remains open to revision as further evidence is collected.')}</p><p>${esc(data.method.adaptation)} <a href="${esc(data.method.url)}" target="_blank" rel="noopener noreferrer">Read the CAST Handbook</a></p><p>Public-source collection is recorded in the evidence ledger; unfilled requests identify the artifact, holder and explanation to test. ${esc(data.scope.evidence_limit)} ${esc(data.scope.temporal_limit)}</p><p>The analysis links ${data.coverage.linked_event_count} events across ${data.coverage.linked_source_episode_count} source episodes. ${esc(data.coverage.scope)} ${esc(data.method.claim)}</p></div><div class="cast-scope-grid">${[['Losses to examine',data.losses],['Hazardous conditions',data.hazards],['Required protections',data.constraints],['Cross-system factors',data.systemic_factors]].map(([label,items])=>`<section><h3>${label}</h3>${cards(items || [])}</section>`).join('')}</div>`;
    }
    function changes() {
      return `<p class="cast-note">These are grouped changes to controls, linked to the underlying events. Implementation and effectiveness are different questions; each change includes its verification needs.</p><h3 class="cast-section-title">Reported changes</h3>${cards(data.improvements.filter(item=>item.kind==='reported_actual'))}<h3 class="cast-section-title">Proposed improvements</h3><p class="cast-note">Analyst proposals for investigation and testing. These are not historical actions or proven optimal interventions.</p>${cards(data.improvements.filter(item=>item.kind==='analyst_proposal'))}`;
    }
    function render() {
      const isPaper = active === 'paper';
      const isWorkbench = active === 'inquiry' || active === 'finding';
      const title = active === 'finding' ? 'A closer look' : sections.find(([key])=>key===active)?.[1] || 'Research notes';
      const body = isWorkbench ? workbench.html(selectedFinding,selectedEvent) : isPaper ? paper() : active === 'findings' ? cards(data.findings) : active === 'evidence' ? researchView() : active === 'structure' ? graph() : active === 'changes' ? changes() : active === 'questions' ? questions() : scope();
      element.innerHTML = `<div class="cast-shell ${isWorkbench?'workbench-shell':isPaper?'paper-shell':'paper-appendix'}">${isWorkbench?'':`<button class="paper-back" data-cast-section="inquiry">Return to the investigation</button>${isPaper?'':`<div class="cast-heading"><h2>${esc(title)}</h2></div>`}`}<div class="cast-content">${body}</div></div>`;
      root.HaruspexReader?.annotate(element.querySelector('.cast-content'));
      wireEvents(element);
      wireInquiry(element);
      if(isWorkbench)workbench.bind(element);
      element.querySelectorAll('[data-cast-section]').forEach(button=>button.addEventListener('click',()=>{
        ...
        const heading=element.querySelector(active==='inquiry'?'.cw-heading h2':active==='paper'?'.paper-title h2':'.cast-heading h2');
        if (heading) { heading.tabIndex=-1; heading.focus({preventScroll:true}); }
      }));
      ...
      if (!isWorkbench && !matchMedia('(prefers-reduced-motion: reduce)').matches) element.querySelector('.cast-content').animate([{opacity:.2},{opacity:1}],{duration:180,easing:'ease-out'});
    }
    render();
