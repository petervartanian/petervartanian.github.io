...
(() => {
  ...
  function mountGateway({scene,getPoints,paintPoint,isReading}){
    ...
    let size,center,radius,glow,grain,backdrop,endpoints=[],dust=[],marble=[],dustColors=[],ratio=1,warmIndex=0,baseGradient=null,surfaceLayer=null,surfaceBmp=null,glowBmp=null,backdropBmp=null,lastPaint=0;
    const dustBitmaps=new Map();
    ...
    function prepare(){
      ...
      dustColors=colors.slice();dustSprites.clear();dustBitmaps.clear();marbleSprites.clear();
      ...
      ratio=canvas.width/size.width;const lx=center.x-radius*.38,ly=center.y-radius*.42,bx=center.x-radius*1.15,by=center.y-radius*1.15,bs=radius*2.3;
      baseGradient=size.ctx.createRadialGradient(lx,ly,radius*.04,center.x,center.y,radius*1.06);baseGradient.addColorStop(0,'#46688a');baseGradient.addColorStop(.5,'#1e3242');baseGradient.addColorStop(1,'#0a1118');
      const boxLayer=()=>{const c=document.createElement('canvas');c.width=c.height=Math.ceil(bs*ratio);const x=c.getContext('2d');x.scale(ratio,ratio);x.translate(-bx,-by);return[c,x];};
      let x;[surfaceLayer,x]=boxLayer();x.globalAlpha=.32;x.fillStyle=x.createPattern(g,'repeat');x.fillRect(bx,by,bs,bs);x.globalAlpha=1;
      const shade=x.createRadialGradient(lx,ly,radius*.2,center.x,center.y,radius*1.02);shade.addColorStop(0,'#05090e00');shade.addColorStop(.6,'#05090e40');shade.addColorStop(1,'#05090ecc');x.fillStyle=shade;x.fillRect(bx,by,bs,bs);
      const spec=x.createRadialGradient(lx+radius*.06,ly+radius*.04,0,lx+radius*.06,ly+radius*.04,radius*.55);spec.addColorStop(0,'rgba(255,255,255,.14)');spec.addColorStop(1,'rgba(255,255,255,0)');x.fillStyle=spec;x.fillRect(bx,by,bs,bs);
      surfaceBmp=null;bitmap(surfaceLayer,b=>{surfaceBmp=b;});
      backdropBmp=null;if(backdrop)bitmap(backdrop,b=>{backdropBmp=b;});
      marbleSprites.clear();marbleBitmaps.clear();for(let c=0;c<dustColors.length;c++){const sprite=marbleSprite(c);bitmap(sprite,b=>{marbleBitmaps.set(c,b);});}
    ...
    function paintSolid(angle,alpha=1){
      ...
      ctx.globalAlpha=alpha;ctx.drawImage(surfaceBmp||surfaceLayer,bx,by,bs,bs);
    ...
    function paintPop(progress){
      ...
      for(const d of dust){const o=d.origin,gone=easeOut((progress-.03-o.order*.5)/.42);if(gone>=1)continue;const w=d.r*o.s*(1-.55*gone)*10/3,key=d.color*4+d.geometry;ctx.globalAlpha=shatter*o.a*(1-gone);ctx.drawImage(dustBitmaps.get(key)||dustSprite(d.color,d.geometry),o.x+o.dx*gone-w/2,o.y+o.dy*gone-w/2,w,w);}
      ctx.globalAlpha=1;
    ...
    function pop({instant=false,at=null}={}){
      ...
      const touch=at||center;
      dust.forEach((d,i)=>{const p=project(d.unit,d.shell,angle);const dist=Math.hypot(p.x-touch.x,p.y-touch.y);d.origin={x:p.x,y:p.y,s:p.s,a:Math.min(1,(.1+.9*Math.pow(p.light,.85))*(.55+.45*p.depth)),order:Math.min(1,dist/(radius*1.35))*(.85+.3*hash(i*3+77)),dx:(hash(i*3+78)-.5)*36,dy:(hash(i*3+79)-.5)*20+16};});
    ...
    button.addEventListener('click',(e)=>{const r=canvas.getBoundingClientRect();pop({at:e.clientX||e.clientY?{x:e.clientX-r.left,y:e.clientY-r.top}:null});});
