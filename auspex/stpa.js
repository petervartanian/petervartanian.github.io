(() => {
  ...
  function pathsGuide(staticMode=false) {
    const paths=`<div class="a1-path-guide"><dl class="a1-path-definitions">${routeDefinitions.map(r=>`<div>${routeSymbol(r.id)}<dt>${esc(r.label)}</dt><dd>${esc(r.definition)}</dd></div>`).join('')}</dl></div>`;
    return staticMode?`${mapOverview()}<h3>Paths</h3>${paths}<h3>Barriers</h3>${proposedBarrierGuide()}<h4>Barrier states</h4><dl class="a1-path-definitions a1-barrier-definitions">${stateDefinitions.map(s=>`<div class="a1-barrier-card">${barrierGlyph(s.id)}<dt>${esc(s.label)}</dt><dd>${esc(s.definition)}</dd></div>`).join('')}</dl><div class="a1-map-instructions">${guide()}</div>`:paths;
  }
  function mapGuide(mode='paths',selected='unknown') {
    const tabs=[['paths','Paths'],['barriers','Barriers']];
    const active=tabs.some(([id])=>id===mode)?mode:'paths';
    return `${mapOverview()}<nav class="a1-guide-tabs" aria-label="Map key sections">${tabs.map(([id,label])=>`<button data-map-guide="${id}" aria-pressed="${id===active}" aria-controls="a1-guide-content">${label}</button>`).join('')}</nav><div id="a1-guide-content">${active==='barriers'?stateGuide(selected):pathsGuide()}</div><div class="a1-map-instructions">${guide()}</div>`;
  }
  function renderMap(selected, a, inspectedBarrier, evidenceMarks = () => '', exploration = false) {
    ...
    const headings={before:'1. Precursors',centre:'2. Event',after:'3. Consequences',recovery:'Recovery'};
  ...
  function controlEvidence() {
    ...
    if (!c) return '';
    return `<section id="stpa-control-evidence" class="stpa-control-evidence" tabindex="-1"><p class="stpa-kicker">${esc(c.kind)} · ${tag(c.constraint)}</p><h3>${esc(c.title)}</h3><p>${esc(c.text)} ${cite([c.source])}</p><p class="stpa-muted">${esc(c.limit)}</p><div class="stpa-limit-types">${c.questions.map(q=>`<article><h4>${esc(q.title)}</h4><p>${esc(q.text)}</p></article>`).join('')}</div></section>`;
  }
  function sourceRegister() {
    return `<ol class="stpa-sources">${model.sources.map((s,index)=>`<li>${sourceMark(s,index)}<div>${esc(s.title)}<p>${esc(s.locator)}. ${esc(s.use)}</p></div></li>`).join('')}</ol>`;
  }
  function methodGuide() {
    const sources=[
      {title:'MIT · System Safety for Health Information Technology (2025)',url:'https://psas.scripts.mit.edu/home/papers/2025_System_Safety_for_Health_Information_Tec.pdf'},
      {title:'MIT · Advanced STPA tutorial',url:'https://psas.scripts.mit.edu/home/wp-content/uploads/2013/04/STPA_advanced_tutorial1.pdf'},
      {title:'UK Civil Aviation Authority · How does bowtie work?',url:'https://www.caa.co.uk/safety-initiatives/working-with-industry/bowtie/about-bowtie/how-does-bowtie-work/'}
    ];
    const mark=index=>sourceMark(sources[index],index);
    return `<section class="a1-method-guide"><h3>STPA in X-oscope</h3><p>Systems-Theoretic Process Analysis examines how control can become inadequate. It begins with the losses to prevent and the hazardous conditions that could produce them. It then maps who controls what, the actions available to them, and the feedback they need.</p><p>Each control action is examined in context. Could harm follow if it is omitted, provided, mistimed, or continued for the wrong duration? Loss scenarios explain how those situations could arise, including how an appropriate action might fail to take effect. The findings inform safety constraints and proposed barriers.${mark(0)}</p><p>Each worked case links selected control actions and loss scenarios to its proposed barriers. The cloud supplies their context. The full research notes contain the control relationships and assumptions behind the map. These examples remain provisional.</p><h3>CAST and bow-tie</h3><p>CAST investigates how control broke down in a reported incident. STPA examines how a future loss could arise. Both draw on STAMP’s view of safety as a control problem.${mark(1)}</p><p>X-oscope uses a bow-tie to arrange contributing threats, a central loss-of-control event, and potential consequences, with barriers along the routes. The map summarizes part of the analysis. Its arrows and numbering are drawing conventions specific to the site.${mark(2)}</p></section>`;
  }
  function guide() {
    return `${overlayGuide()}${methodGuide()}`;
  }
  function research() {
    return `<ul class="a1-sources">${model.sources.map((s,index)=>`<li>${sourceMark(s,index)}<div>${esc(s.title)}</div></li>`).join('')}</ul><p><a href="pathways.html#${esc(model.pathway)}">Full research notes ↗</a></p>`;
  }
  function staticPage() {
