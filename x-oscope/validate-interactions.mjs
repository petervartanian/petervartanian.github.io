...
let url='file:///fixture/x-oscope/index.html';
...
for(const m of models) {
 url=`file:///fixture/x-oscope/index.html?p=${m.displayId}`;
 test.readLocation();
...
url='file:///fixture/x-oscope/index.html?p=A.1&overlay=maybe&i=AFK-2024';test.readLocation();
...
for(const p of context.window.AuspexData.pathways.filter(p=>!stpa.has(p.id))) {
 url='file:///fixture/x-oscope/index.html?p='+p.displayId+'&i=AFK-2024&explore=1';test.readLocation();
 assert.equal(test.state.incident,'');assert.equal(test.state.exploration,false);
...
for(const m of models) {
 url=`file:///fixture/x-oscope/index.html?p=${m.displayId}`;test.readLocation();
 const before=url;
...
for(const m of models) {
 for(const incident of Object.keys(m.presentation.overlays)) {
  url=`file:///fixture/x-oscope/index.html?p=${m.displayId}&i=${incident}`;test.readLocation();
  const a=context.window.AuspexData.assessments.find(a=>a.pathway===m.pathway && a.incident===incident);
